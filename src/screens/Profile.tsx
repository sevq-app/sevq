  ...
  return (
    <div className="h-full overflow-y-auto pb-28 md:pb-0">
      ...
      <div className="px-4 sm:px-6 space-y-4 max-w-2xl mx-auto">
        <section className="bg-[var(--bg-card)] rounded-2xl p-5 flex items-center justify-between shadow-[0_8px_24px_rgba(101,70,199,0.08)]"><div><h3 className="font-heading font-bold">Статус</h3><p className="text-sm text-sevchik-textSecondary font-body mt-0.5 flex items-center gap-1.5"><span className={`w-2 h-2 rounded-full ${online ? 'bg-sevchik-mint' : 'bg-sevchik-textSecondary/50'}`} />{online ? 'В сети' : 'Не в сети'}</p></div><button onClick={() => setOnline(!online)} aria-label="Изменить статус" className={`relative w-14 h-8 rounded-full transition-colors duration-300 ${online ? 'bg-sevchik-mint' : 'bg-sevchik-textSecondary/20'}`}><span className={`absolute top-1 w-6 h-6 rounded-full bg-white transition-[left] ${online ? 'left-7' : 'left-1'} shadow-[0_2px_8px_rgba(0,0,0,0.15)]`} /></button></section>
        <button onClick={() => onNavigate?.('about')} className="w-full bg-[var(--bg-card)] rounded-2xl p-5 flex items-center justify-between text-left shadow-[0_8px_24px_rgba(101,70,199,0.08)]"><span className="font-heading font-bold">Укажите информацию о себе</span><ChevronRight size={20} className="text-sevchik-textSecondary" /></button>

        <section className="bg-[var(--bg-card)] rounded-2xl p-5 shadow-[0_8px_24px_rgba(101,70,199,0.08)]"><button onClick={() => onNavigate?.('photos')} className="w-full flex items-center justify-between mb-3 text-left"><h3 className="font-heading font-bold">Фотографии</h3><ChevronRight size={20} className="text-sevchik-textSecondary" /></button>{photos.length === 0 ? <p className="text-sm text-sevchik-textSecondary font-body">Фото ещё не загрузили</p> : <div className="grid grid-cols-3 gap-2">{photos.slice(0, 6).map((photo) => <button key={photo} onClick={() => onNavigate?.('photos')} className="aspect-square overflow-hidden rounded-xl"><img src={photo} alt="" className="w-full h-full object-cover" /></button>)}</div>}</section>

        <section className="bg-[var(--bg-card)] rounded-2xl p-5 shadow-[0_8px_24px_rgba(101,70,199,0.08)]"><button onClick={() => onNavigate?.('my-groups')} className="w-full flex items-center justify-between text-left"><h3 className="font-heading font-bold">Мои группы</h3><ChevronRight size={20} className="text-sevchik-textSecondary" /></button>{groups.length === 0 ? <p className="text-sm text-sevchik-textSecondary mt-4">Вы ещё не состоите в группах</p> : <><div className="grid grid-cols-3 gap-3 mt-4">{groups.slice(0, 3).map((group) => <button key={group.id} onClick={() => onNavigate?.('my-groups')} className="min-w-0 text-left"><GroupIcon group={group} /><span className="block text-xs font-heading font-bold truncate mt-2">{group.name}</span></button>)}</div><button onClick={() => onNavigate?.('my-groups')} className="mt-4 text-sm font-heading font-bold text-sevchik-purple">Показать все</button></>}</section>

        <button onClick={() => undefined} className="w-full bg-[var(--bg-card)] rounded-2xl p-5 flex items-center gap-3 text-left shadow-[0_8px_24px_rgba(101,70,199,0.08)]"><Sparkles size={22} className="text-sevchik-purple" /><span className="font-heading font-bold">Стикеры</span></button>
        <button onClick={() => { setInviteMode('menu'); setInviteOpen(true); }} className="w-full bg-[var(--bg-card)] rounded-2xl p-5 flex items-center gap-3 text-left shadow-[0_8px_24px_rgba(101,70,199,0.08)]"><Link size={23} className="text-[#6546C7]" /><span className="flex-1 font-heading font-bold">Пригласить друзей</span><ChevronRight size={20} className="text-sevchik-textSecondary" /></button>
        <div className="pb-6" />
      </div>
