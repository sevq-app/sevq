...
import { writeAvatarPhoto } from '@/lib/avatarPhoto';
...
export function Photos({ onBack }: { onBack: () => void }) {
  ...
  return (
    <div className="h-full overflow-y-auto px-4 sm:px-6 pt-6 pb-28 max-w-2xl mx-auto">
      <button onClick={onBack} className="flex items-center gap-2 text-sevchik-textSecondary mb-8"><ArrowLeft size={20} /> Назад</button>
      <h1 className="font-heading font-extrabold text-2xl mb-5">Фотографии</h1>
      {photos.length ? (
        <div className="grid grid-cols-3 gap-2">
          {photos.map((photo) => (
            <button key={photo} onClick={() => setSelected(photo)} className="aspect-square overflow-hidden rounded-xl">
      ...
      {selected && (
        <div className="fixed inset-0 z-50 bg-black/80 flex flex-col items-center justify-center p-5" onClick={() => setSelected(null)}>
          ...
          <button
            onClick={(event) => { event.stopPropagation(); setCropSource(selected); }}
